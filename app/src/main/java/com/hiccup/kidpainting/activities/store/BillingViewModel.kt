...
import com.hiccup.kidpainting.utilities.billing.deviceHasGooglePlaySubscription
import com.hiccup.kidpainting.utilities.billing.serverHasSubscription
...
class BillingViewModel(application: Application) : AndroidViewModel(application) {
     ...
     */
    val purchases = (application as PaintingApplication).billingClientLifecycle.purchases

    /**
     * ProductDetails for all known Products.
     */
    private val productsWithProductDetails =
        (application as PaintingApplication).billingClientLifecycle.productsWithProductDetails


    /**
     * Subscriptions record according to the server.
     */
    val subscriptions: StateFlow<List<SubscriptionStatus>> = (application as PaintingApplication).repository.subscriptions
    ...
    fun openPlayStoreSubscriptions() {
        val hasBasic = deviceHasGooglePlaySubscription(purchases.value, BillingConstants.BASIC_PRODUCT)
        val hasPremium = deviceHasGooglePlaySubscription(purchases.value, BillingConstants.BASIC_PRODUCT)
        Log.d(TAG, "hasBasic: $hasBasic, hasPremium: $hasPremium")
        when {
            hasBasic && !hasPremium -> {
                // If we just have a basic subscription, open the basic Product.
                openPlayStoreSubscriptionsEvent.postValue(BillingConstants.BASIC_PRODUCT)
            }
            !hasBasic && hasPremium -> {
                // If we just have a premium subscription, open the premium Product.
                openPlayStoreSubscriptionsEvent.postValue(BillingConstants.PREMIUM_PRODUCT)
            }
            else -> {
    ...
    fun openSubscriptionPageOnGooglePlay() {
        subscriptions.value.let { subscriptionStatusList ->
            when {
                serverHasSubscription(subscriptionStatusList, BillingConstants.PREMIUM_PRODUCT) ->
                    openProductPlayStoreSubscriptions(BillingConstants.PREMIUM_PRODUCT)
                serverHasSubscription(subscriptionStatusList, BillingConstants.BASIC_PRODUCT) ->
                    openProductPlayStoreSubscriptions(BillingConstants.BASIC_PRODUCT)
            }
        }
    }
