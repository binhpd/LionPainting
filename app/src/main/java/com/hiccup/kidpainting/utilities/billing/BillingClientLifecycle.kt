...
class BillingClientLifecycle private constructor(
    ...
    ProductDetailsResponseListener, PurchasesResponseListener, PurchaseHistoryResponseListener {
    ...
    fun queryHistoryPurchases() {
        if (!billingClient.isReady) {
            Log.e(TAG, "queryHistoryPurchases: BillingClient is not ready")
            billingClient.startConnection(this)
        }
    ...
    private fun processPurchaseHistory(purchasesList: List<PurchaseHistoryRecord>?) {
        Log.d(TAG, "processPurchasesHistory: ${purchasesList?.size} purchase(s)")
    }
    ...
    companion object {
        ...
        private val LIST_OF_PRODUCTS = listOf(
            BillingConstants.BASIC_PRODUCT,
            BillingConstants.PREMIUM_PRODUCT,
        )
